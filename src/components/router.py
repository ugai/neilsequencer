...
class RouteView(Gtk.DrawingArea):
    ...
    def get_plugin_at(self, xxx_todo_changeme4):
        ...
        for mp in reversed(list(player.get_plugin_list())):
            ...
            if (mx >= (x - PW)) and (mx <= (x + PW)) and (my >= (y - PH)) and (my <= (y + PH)):
                if sum(tuple(Gdk.Rectangle(x - PW + LEDOFSX, y - PH + LEDOFSY, LEDWIDTH, LEDHEIGHT).intersect(Gdk.Rectangle(mx, my, 1, 1)))):
                    area = AREA_LED
                return mp, (x, y), area
    ...
    def on_motion(self, widget, event):
        ...
        """
        window = self.get_window()
        _, x, y, state = window.get_pointer()
        if self.dragging:
            player = com.get('neil.core.player')
            ...
            if res:
                mp, (mx, my), area = res
                window.set_cursor(Gdk.Cursor(Gdk.HAND1) if area == AREA_LED else None)
        return True
    ...
    def on_draw_led_timer(self):
        ...
        if window is not None:
            ...
            for mp, (rx, ry) in ((mp, get_pixelpos(*mp.get_position())) for mp in player.get_plugin_list()):
                rx, ry = rx - PW, ry - PH
                window.invalidate_rect(Gdk.Rectangle(int(rx), int(ry), PLUGINWIDTH, PLUGINHEIGHT), False)
        return True
    ...
    def draw_leds(self):
        ...
        for mp, (rx, ry) in ((mp, get_pixelpos(*mp.get_position())) for mp in player.get_plugin_list()):
            ...
            if not pi.plugingfx:
                pi.plugingfx = Gdk.Pixmap(window, PLUGINWIDTH, PLUGINHEIGHT, -1)
...
#                    gc.set_foreground(cm.alloc_color(brushes[self.COLOR_DEFAULT]))
            ...
            window.draw_drawable(gc, pi.plugingfx, 0, 0, int(rx), int(ry), -1, -1)
    ...
    def draw(self, ctx):
        ...
        if not self.routebitmap:
            self.routebitmap = Gdk.Pixmap(window, w, h, -1)
            ...
            for mp, (rx, ry) in mplist:
                ...
                for index in range(mp.get_input_connection_count()):
                    ...
                    draw_line_arrow(bmpctx, arrowcolors[mp.get_input_connection_type(index)], int(crx), int(cry), int(rx), int(ry))
        gc = window.new_gc()
        window.draw_drawable(gc, self.routebitmap, 0, 0, 0, 0, -1, -1)
        if self.connecting:
