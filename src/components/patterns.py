...
from neil.utils import roundint
PATLEFTMARGIN = 48
CONN = 0
GLOBAL = 1
TRACK = 2
...
class PatternToolBar(Gtk.HBox):
    ...
    def __init__(self, pattern_view):
        ...
        self.pack_start(self.pluginselect, expand=False, fill=False, padding=0)
        self.pack_start(self.patternselect, expand=False, fill=False, padding=0)
        self.pack_start(self.waveselect, expand=False, fill=False, padding=0)
        self.pack_start(Gtk.VSeparator(), expand=False, fill=False, padding=0)
        self.pack_start(self.octavelabel, expand=False, fill=False, padding=0)
        self.pack_start(self.octaveselect, expand=False, fill=False, padding=0)
        self.pack_start(self.edit_step_label, expand=False, fill=False, padding=0)
        self.pack_start(self.edit_step_box, expand=False, fill=False, padding=0)
        self.pack_start(self.playnotes, expand=False, fill=False, padding=0)
        self.pack_start(Gtk.VSeparator(), expand=False, fill=False, padding=0)
        self.pack_start(self.btnhelp, expand=False, fill=False, padding=0)
    ...
    def get_plugin_source(self):
        ...
        plugins = sorted(list(player.get_plugin_list()), key=functools.cmp_to_key(cmp_func))
...
class PatternPanel(Gtk.VBox):
    ...
    def __init__(self):
        ...
        label = Gtk.Label()
        self.statuslabels.append(label)
        self.statusbar.pack_end(label, expand=False, fill=False, padding=0)
        ...
        self.pack_start(scrollwin, expand=False, fill=False, padding=0)
...
#                       return True
    ...
    def on_key_down(self, widget, event):
        ...
        player = com.get('neil.core.player')
        eventbus = com.get('neil.core.eventbus')
        # shiftdown = mask & Gdk.SHIFT_MASK
        # ctrldown = mask & Gdk.CONTROL_MASK
        if k == 'less':
            player.activate_wave(-1)
            pass
        elif k == 'greater':
            player.activate_wave(1)
        elif mask & Gdk.SHIFT_MASK and k == 'Down':
            if not self.selection:
                self.selection = self.Selection()
            ...
            self.adjust_selection()
            self.redraw()
        elif mask & Gdk.SHIFT_MASK and k == 'Up':
            if not self.selection:
                self.selection = self.Selection()
            if self.shiftselect == None:
                self.shiftselect = self.row
            ...
            self.adjust_selection()
            self.redraw()
        elif mask & Gdk.SHIFT_MASK and (k == 'Right' or k == 'Left'):
            if not self.selection:
                self.selection = self.Selection()
            if self.shiftselect == None:
                self.shiftselect = self.row
